  ...
  <div v-else class="shopping-cart m-l-lg m-r-lg m-t-lg">
    <div class="m-b-lg">
      <b-table :bordered="true" :data="tableItems">
        <template slot-scope="props">
          <b-table-column field="name" label="Nume produs">{{ props.row.title }}</b-table-column>
          <b-table-column field="category" label="Categorie">{{ props.row.category.title }}</b-table-column>
          <b-table-column
            width="200"
            field="subcategory"
            label="Subcategorie"
          >{{ props.row.subcategory.title }}</b-table-column>
          <b-table-column field="u_m" label="U.M.">{{ props.row.quantity_type }}</b-table-column>
    ...
    <div class="payment-panel card p-lg m-t-lg m-b-md">
      ...
      <card
        ...
        @change="setComplete"
      />
      <b-button class="pay-with-stripe" :disabled="!complete" @click="pay">Plătește acum</b-button>
    </div>

    <b-modal :active.sync="deleteProductModal.visible" has-modal-card :can-cancel="true">
      <div class="card p-md modal-content">
        <h3 class="m-b-lg">Sunteți siguri că doriți să ștergeți acest produs?</h3>
        <div class="flex-align">
          <b-button type="is-primary" @click="cancelDeleteProduct">Anulează</b-button>
          <b-button type="is-danger" @click="deleteProductFromCart">Confirmă</b-button>
        </div>
      </div>
    </b-modal>
  </div>
